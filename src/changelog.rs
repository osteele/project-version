...
pub fn update_changelog(path: &Path, version: &Version) -> Result<()> {
    ...
    for pattern in &UNRELEASED_PATTERNS {
        ...
        if re.is_match(&content) {
            ...
            found = true;
            debug!("Matched pattern: {} - replacing with: {}", pattern, version_header);
            break;
        }
    }
    
    if !found {
        warn!("No unreleased section found in changelog at {}", path.display());
        // We'll just keep the file as is to avoid incorrect modifications
    } else {
        // Write the updated content back to the file
        fs::write(path, new_content)
            .context("Failed to write updated changelog")?;
        debug!("Updated unreleased section to version {} in {}", version, path.display());
    }
...
pub fn dry_run_update_changelog(path: &Path, version: &Version) -> Result<String> {
    ...
    for pattern in &UNRELEASED_PATTERNS {
        ...
        if let Some(m) = re.find(&content) {
            ...
            diff = format!("Would update changelog {}:\n  {} → {}", 
                          path.display(), header.trim(), version_header);
            found = true;
            break;
        }
    }
    ...
    if !found {
        diff = format!("No unreleased section found in changelog {}", path.display());
    }
