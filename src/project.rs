...
impl NodeProject {
    ...
    fn update_version_internal(&self, version: &Version, dry_run: bool) -> Result<String> {
        ...
        let diff = format!("{}:\n  version: {} → {}", 
                          if dry_run { "Would update package.json" } else { "Updated package.json" },
                          old_version, version);
...
impl Project for NodeProject {
    fn get_version(&self) -> Result<Version> {
        ...
        Version::parse(version_str)
            .context("Failed to parse version from package.json")
    }
    ...
    fn update_version_internal(&self, version: &Version, dry_run: bool) -> Result<String> {
        ...
        if !dry_run {
            ...
            fs::write(&self.path, new_content)
                .context("Failed to write updated pyproject.toml")?;
        }
...
impl Project for PythonProject {
    fn get_version(&self) -> Result<Version> {
        ...
        Version::parse(version_str)
            .context("Failed to parse version from pyproject.toml")
    }
            ...
            if let Some(package) = doc.get_mut("package") {
                if let Some(package_table) = package.as_table_mut() {
                    if package_table.contains_key("version") {
                        ...
                        return Err(anyhow!("No version field found in Cargo.toml package table"));
                    }
                }
            } else {
...
impl Project for RustProject {
    fn get_version(&self) -> Result<Version> {
        ...
        let version_str = toml_value.get("package")
            .and_then(|package| package.get("version"))
            .and_then(|v| v.as_str())
            .ok_or_else(|| anyhow!("No version field found in Cargo.toml"))?;
        
        Version::parse(version_str)
            .context("Failed to parse version from Cargo.toml")
    }
...
impl GoProject {
    ...
    fn update_version_internal(&self, version: &Version, dry_run: bool) -> Result<String> {
        ...
        diff.push_str(&format!("{} Go project version from {} to {}:\n",
        ...
        for file_path in &version_files {
            ...
            let re = regex::Regex::new(r#"((?:Version|VERSION)\s*=\s*["'])v?([0-9]+\.[0-9]+\.[0-9]+)(["'])"#)
                .unwrap();
            
            if re.is_match(&file_content) {
                diff.push_str(&format!("  File: {}\n", file_path.display()));
                
                let new_content = re.replace(&file_content, |caps: &regex::Captures| {
...
impl Project for GoProject {
    fn get_version(&self) -> Result<Version> {
        // For simplicity in this implementation, we'll check for a version.go file
        let version_files = self.get_version_files();
        
        for file_path in &version_files {
            let version_content = fs::read_to_string(file_path)
                .context("Failed to read version file")?;
            
            // Try to match version string in various formats
            let re = regex::Regex::new(r#"(?:Version|VERSION)\s*=\s*["']v?([0-9]+\.[0-9]+\.[0-9]+)["']"#)
                .unwrap();
            
            if let Some(captures) = re.captures(&version_content) {
...
impl RubyProject {
    ...
    fn find_version_rb_file(&self) -> Option<PathBuf> {
        ...
        if lib_dir.exists() {
                ...
                if let Ok(content) = fs::read_to_string(gemspec_path) {
                    ...
                    if let Some(caps) = name_re.captures(&content) {
                        if let Some(name) = caps.get(1) {
                            let version_path = lib_dir.join(name.as_str()).join("version.rb");
            ...
            let direct_version_path = lib_dir.join("version.rb");
    ...
    fn update_version_internal(&self, version: &Version, dry_run: bool) -> Result<String> {
        ...
        let prefix = if dry_run { "Would update" } else { "Updated" };
        diff.push_str(&format!("{} Ruby project version from {} to {}:\n", 
                              prefix, old_version, version));
        ...
        if let Some(ref path) = version_rb_path {
            ...
            if version_re.is_match(&content) {
                ...
                let new_content = version_re.replace(&content, |caps: &regex::Captures| {
                    format!("VERSION = '{}'{}",
                           version,
                           if let Some(m) = caps.get(2) { m.as_str() } else { "" })
                });
